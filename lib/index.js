...
function restartClientScript(avoidAppButtons) {
  ...
  // 按钮垂直位置：必须和 dsh 自己的顶部控件在同一条中线上，否则看起来像浮在控件下面。
  //   dsh 应用顶栏（对话/文件/窗口那一条）height:36px
  //   右侧文档预览面板头部（TextPreview.module.css 的 .header）height:38px
  // 两者都从 top:0 起算、控件垂直居中 → 取 36 这一档：
  //   中心 = top + 30/2，要等于 36/2 = 18，所以 top:3px。对 38px 的面板头只差 1px。
  //
  // ⚠️ 这个值写在下面 ensureStyle 的 CSS 字符串里（top:3px 与 height:30px），
  // 这里是字符串拼接而不是模板插值，**别**把它抽成常量再拼进来 —— 本函数体整体是
  // 一段模板字面量，用引号加号的写法在里面只是普通字符、不会被求值，生成出来的 CSS
  // 会原样带上那段源码，浏览器直接忽略、按钮掉回默认位置。两个注入脚本各有一份，
  // 改的时候一起改；client-test 的 S25 那组断言就是钉这个的。
  // （注：这段注释本身也在模板字面量里，所以正文里不能出现反引号。）
...
function updateClientScript(canApply) {
  ...
  function ensureStyle(){ if($("#dsh-dl-update-style")) return; var st=mk("style"); st.id="dsh-dl-update-style";
    st.textContent =
      // top:3px 必须和 restartClientScript 里那个按钮保持一致（两段脚本各自独立，
      // 谁也不认识谁）。3 = (36 - 30) / 2，让按钮垂直中心压在 dsh 顶栏中线上。
      "#dsh-dl-upd{position:fixed;top:3px;right:10px;z-index:2147483000;display:inline-flex;align-items:center;gap:6px;height:30px;padding:0 12px;border-radius:16px;font:13px/1 system-ui,'Segoe UI','Microsoft YaHei UI',sans-serif;cursor:pointer;border:1px solid;user-select:none;opacity:.9;transition:opacity .15s,right .18s ease;}" +
      "#dsh-dl-upd:hover{opacity:1}" +
      "#dsh-dl-upd .v{font-weight:700}" +
      // 面板顶端位置跟着按钮走：按钮占 3..33，留 7px 间隙 → 40。
      "#dsh-dl-upd-pop{position:fixed;top:40px;right:12px;z-index:2147483002;display:none;box-sizing:border-box;width:min(340px,92vw);padding:14px 16px;border-radius:12px;border:1px solid;font:13px/1.7 system-ui,'Segoe UI','Microsoft YaHei UI',sans-serif;box-shadow:0 10px 40px rgba(0,0,0,.28);}" +
