...
function restartClientScript(avoidAppButtons) {
   ...
   *
   * 从右缘往左扫，并容忍簇内的小空隙 —— 真实的顶栏是一排按钮，按钮之间本来就有
   * 几像素的缝，遇到缝就停会把「多个按钮」误判成一个。空隙超过 GAP_TOL 才认为
   * 这个簇结束了（避免把左边缘那些不相干的元素也算进来）。
   */
  function appRightEdge(){
    var vw2 = vw(); if(!vw2) return null;
    var STEP = 6;
    var SCAN_W = Math.min(Math.round(vw2 * 0.75), 900);
    var ys = [16, 24, 32];                  // 覆盖我们按钮的纵向范围（top:10 / 高 30）
    var GAP_TOL = 40;                       // 簇内容忍的最大空隙
    var leftMost = null, sinceCovered = 0, sawAny = false;
    for(var d=2; d<=SCAN_W; d+=STEP){
      var x = vw2 - d;
      if(x <= 0) break;
      var covered = false;
      for(var k=0;k<ys.length;k++){ if(compactAppAt(x, ys[k], vw2)){ covered = true; break; } }
      if(covered){ sawAny = true; leftMost = x; sinceCovered = 0; }
      else {
        sinceCovered += STEP;
        if(sawAny && sinceCovered > GAP_TOL) break;   // 簇结束
      }
    }
    return sawAny ? leftMost : null;
  }
