 ...
 * 版本可以和壳不一致（本机壳 rc.1、应用 rc.2）。用 createRequire 从入口解析走的是 Node
 * 标准解析（含 pnpm 软链），不猜目录布局；解析不到返回 null，由调用方回退到 CLI 版本。
 */
  ...
  return new Promise((res, rej) => {
    const req = https.get(
      url,
      {
        ...
        // node core 的 https 不读 http_proxy 环境变量，这里是直连 registry（本机已验证可达）。
        timeout: timeoutMs
      },
      (resp) => {
 ...
 * - attempts：网络抖动的重试次数（默认 3）。**这条是必须的**：实测本机到 npm 的
 *   连接会偶发超时/ECONNRESET（并发 8 个请求时有 2 个挂掉），而检查只跑一次；
